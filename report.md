...
Traffic sign recognition is an important component of modern driver assistance systems and autonomous driving pipelines. In real-world applications, a classifier must recognize signs under changing illumination, partial occlusion, motion blur, and different viewing distances — conditions that directly affect how much visual information is available in an image. Because traffic signs communicate legally and safety-relevant instructions, misclassifying a speed limit, priority sign, or stop sign could lead to incorrect driving decisions. This makes reliable classification not just a technical challenge, but a safety-relevant one.

To study this problem in a controlled and reproducible way, we use the German Traffic Sign Recognition Benchmark (GTSRB) — a widely used standard dataset for traffic sign classification. A detailed description of the dataset, including its class distribution and visual properties, is provided in Section 2.
